...
import com.example.mygame2048.Activity.Activity.MainActivity;

import java.util.ArrayList;
import java.util.List;

import bean.GameItem;
...
public class GameView extends GridLayout implements OnTouchListener {
    ...
    public void revertGame() {
        ...
        if (sum != 0) {
            MainActivity.getGameActivity().setScore(mScoreHistory, 0);
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (isMoved()) {
                    addRandomNum();
                    // 修改显示分数
                    MainActivity.getGameActivity().setScore(Config.SCROE, 0);
                }
                checkCompleted();
                break;
            default:
    ...
    private void checkCompleted() {
        ...
        if (result == 0) {
            if (Config.SCROE > mHighScore) {
                ...
                editor.apply();
                MainActivity.getGameActivity().setScore(Config.SCROE, 1);
                Config.SCROE = 0;
            }
